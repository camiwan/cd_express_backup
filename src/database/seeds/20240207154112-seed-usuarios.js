...
module.exports = {
  async up async (queryInterface, Sequelize) {
    ...
    for (let i = 0; i < 5; i++) {
      usuarios.push({
        ...
        password_hash: bcryptjs.hash(`senha${i + 1}`, 8),
        empresa_id: empresas[i % empresas.length].id, // Associar a empresa de acordo com o índice
        created_at: new Date(),
        updated_at: new Date(),
      });
    }
